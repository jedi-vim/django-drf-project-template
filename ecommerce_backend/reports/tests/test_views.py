...
@pytest.mark.parametrize("params,response_code,response_body",
                         [({'salesperson_id': 1,
                           ...
                           {'salesperson_id': 1,
                             ...
                             ({},
                              400,
                              {'salesperson_id': 'Salesperson ID not provided'}),
                             ({'salesperson_id': 'wrongID'},
                              400,
                              {'salesperson_id': 'wrongID is a invalid ID'})])
def test_get_commission_per_salesperson(
        app, params, response_code, response_body):
...
def test_get_sellable_purchased_by_consumer(
        app, params, response_code, response_body):
    ...
    with freeze_time("2020-09-02 12:00:01"):
        ...
        item_4 = PurchaseItemFactory(sellable__description='sellable_4',
                                     sellable__price=decimal.Decimal('1.99'),
                                     purchase__customer=customer_2,
                                     purchase__closed=True, quantity=2)
        item_4.save()

        item_5 = PurchaseItemFactory(sellable=item_1.sellable,
                                     purchase__customer=customer_2,
                                     quantity=3)
        item_5.save()
