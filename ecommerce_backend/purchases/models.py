...
class Storable(models.Model):
    sellable = models.OneToOneField(
        Sellable, on_delete=models.CASCADE, null=True, related_name="storable"
    )
    ...
    def decrease(self, quantity: int):
        if quantity > self.quantity_available:
            ...
            raise InsufficientStockError(
                f"Sellable {sellable_name} canno't be decreased by {quantity}"
            )
...
class Purchase(models.Model):
    ...
    status = models.CharField(default=OPENED, max_length=15, choices=STATUSES)
    created_at = models.DateTimeField(auto_now_add=True)
    closed_at = models.DateTimeField(null=True)
    customer = models.ForeignKey(
        Person, on_delete=models.SET_NULL, null=True, related_name="customer_purchases"
    )
    ...
    def add_item(self, sellable: Sellable, quantity: int):
        ...
        item = PurchaseItem.create(purchase=self, sellable=sellable, quantity=quantity)
        item.save()
...
class PurchaseItem(models.Model):
    ...
    sellable = models.ForeignKey(Sellable, on_delete=models.SET_NULL, null=True)
